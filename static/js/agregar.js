...
window.addEventListener("DOMContentLoaded", () => { // corre el código cuando carga la pagina
    ...
    function addContacto() {
        const actuales = contactar_detalle.querySelectorAll(".bloque-contacto").length;
        if (actuales >= 5) return; // máximo 5

        const bloque = document.createElement("div");
        bloque.classList.add("bloque-contacto");

        // select
        const sel = document.createElement("select");
        sel.name = "contactar_nombre";
        ["", "Whatsapp", "Telegram", "X", "Instagram", "Tiktok", "Otra"].forEach((m,i) => {
            const opt = document.createElement("option");
            opt.value = m;
            opt.textContent = i === 0 ? "Seleccione medio de contactar" : m;
            if (i === 0) { opt.disabled = true; opt.selected = true; }
            sel.appendChild(opt);
        });
        bloque.appendChild(sel);

        // input que aparece al elegir
        const input = document.createElement("input");
        input.type = "text";
        input.name = "contactar_identificador";
        ...
        sel.addEventListener("change", () => {
            if (sel.value) {
                ...
                // si este es el último bloque y todavía hay menos de 5, agregamos otro
                if (contactar_detalle.lastChild === bloque) {
                    addContacto();
                }
            } else {
                input.style.display = "none";
        ...
        contactar_detalle.appendChild(bloque);
    }

    // al cargar, agregamos el primer bloque
    addContacto();
    ...
    tipo.addEventListener("change", () => {
        if (tipo.value === "otro") {
        tipo_extra.innerHTML = '<input type="text" id="tipo-otro" name="tipo_otro" placeholder="Especifique el tipo" required>';
        } else {
    ...
    document.getElementById('btn-cerrar-error').addEventListener('click', () => {
        document.getElementById('error-popup').classList.remove('activo');
    });
    ...
    form.addEventListener("submit", (e) => {
        ...
        // validar contactos
        const bloques = document.querySelectorAll("#contactar_detalle .bloque-contacto");
        ...
        if (errores.length > 0) {
            ...
            listaErrores.innerHTML = errores.map(error => `<li>${error}</li>`).join('');
            document.getElementById('error-popup').classList.add('activo');
            return;
        }
